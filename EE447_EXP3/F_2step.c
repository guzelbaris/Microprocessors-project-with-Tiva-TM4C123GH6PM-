...
void GPIOB_Handler(void)
{
	if(IN==0xE0){		// clockwise direction
		...
		if(OUT==8){	//note that in clock wise direction Full step output 4 follow Full step output 1 
			OUT = 0x01; 
		}
		else
   OUT=OUT  << 1; // logical binary left shift to arrange output step from 4 to 1
			GPIOB->IM &= 0x0F;
	GPIOB->ICR |= 0xF0;
	}
  }
